...
class SMax():
    def __init__(self):
        ...
        out = tf.layers.conv2d(inputs=out,filters=48,kernel_size=[3,3],padding='same',activation=tf.nn.softmax,strides=2)
        #out = tf.layers.max_pooling2d(inputs=out,pool_size=[2,2],padding='same',strides=2)
        out = tf.reshape(out,[-1,np.prod(out.get_shape().as_list()[1:])])
        
        out = tf.layers.dense(inputs=out,units=3000,activation=tf.nn.relu)
        ...
        self.out = tf.layers.dense(inputs=feat,units=n_classes)
